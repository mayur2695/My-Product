({
    init: function(component, event, helper) {
        ...
        empApi.onError($A.getCallback(error => {
            ...
            var MessageData = JSON.stringify(message.data['sobject']);
            if(namespace != null && namespace != "c") {
              MessageData = MessageData.replace(new RegExp(namespace +"__", 'g'), "");
              console.log(MessageData);
            }
            MessageData = JSON.parse(MessageData);
            
            if(AllObjData[x].Id == MessageData.Id ){
            //For massSync Button disable and enable According to Mass Sync Checkbox
            //
            
            
            AllObjData[x].GYNCF_Mass_Sync__c = MessageData.GYNCF_Mass_Sync__c;
            
            // Display Progress Bar
            if(MessageData.GYNCF_Mass_Sync_Live__c == true){
            AllObjData[x].GYNCF_Mass_Sync_Live__c = true;
            var ToBeSyncedStrm = MessageData.GYNCF_To_Be_Synced__c;
            var SyncedStrm = MessageData.GYNCF_Synced__c;
